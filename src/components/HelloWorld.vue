  ...
  export default {
    ...
    methods: {
      ...
      modelling() { //开始建立模型
        ...
        var img = textureLoader.load(require('../../static/image/home3.jpeg'));
      ...
      changeScene(type) {
        ...
        var textureLoader = new THREE.TextureLoader(); //创建纹理贴图		
        if (type == 'enter') {
          img = textureLoader.load(require('../../static/image/home1.jpg')); //vue加载图表需要用 require形式
          canvasText = this.getcanvers('返回'); //生成一个canvers 文字图案对象	
          names = '返回';
        } else if (type == 'backtrack') { //返回房间
          img = textureLoader.load(require('../../static/image/home3.jpeg')); //vue加载图表需要用 require形式	
