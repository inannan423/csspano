	...
	export default {
		...
		methods: {
			...
			modelling() {
				...
				var img = textureLoader.load(require('../../public/img/jia.jpeg'));//第一个出现的场景的全景图
				//加载全景图资源
				...
				meshText.name = '闪现';
				//meshText.position.set(-50, 20, -20);
				...
				var img = texture1.load(require('../../public/img/explain2-1.png'));//第一个出现的场景的全景图
				...
				var img = texture2.load(require('../../public/img/explain2-2.png'));//第一个出现的场景的全景图
				...
				var img = texture3.load(require('../../public/img/explain2-3.png'));//第一个出现的场景的全景图
			...
			changeScene(type) {
				...
				var textureLoader = new THREE.TextureLoader(); //创建纹理贴图		
				if (type == 'enter') {
					img = textureLoader.load(require('../../public/img/港珠澳.jpg')); //vue加载图表需要用 require形式
					sceneName = 'sanya';//改变场景名
					canvasText = this.getcanvers('返回'); //生成一个canvers 文字图案对象	
					names = '返回';
				} else if (type == 'backtrack') { //返回房间
					img = textureLoader.load(require('../../public/img/jia.jpeg')); //vue加载图表需要用 require形式
					sceneName = 'jia';//改变场景名	
