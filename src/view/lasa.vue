    ...
    export default {
        ...
        methods: {
            ...
            css_model() {
                ...
                var img = textureLoader.load(require('../../public/img/lasa.jpg'));
                //加载全景图资源
                ...
                var canvasText = this.getcanvers('三亚'); //生成一个canvers 文字图案对象
                ...
                var canvasText1 = this.getcanvers('盐城'); //生成一个canvers 文字图案对象
                var texture1 = new THREE.CanvasTexture(canvasText1);
                var geometryText1 = new THREE.PlaneGeometry(16, 10, 60, 60);
                var materialText1 = new THREE.MeshPhongMaterial({
                    map: texture1, // 设置纹理贴图
                    side: THREE.DoubleSide,
                });
                var meshText1 = new THREE.Mesh(geometryText1, materialText1);
                meshText1.name = '盐城';
                meshText1.position.set(-20, 30, 50);
            ...
            getcanvers(text) { //生成一个canvers图案
                ...
                // 矩形区域填充背景
                cans.fillStyle = "rgba(148,213,148,0.5)";
                cans.backgroundColor = "rgba(255,215,0,0.3)"; //背景颜色
                cans.border = "thin dotted #FF0000";
                cans.borderRadius = "40px";
                cans.fillRect(0, 0, 500, 300); //生成矩形
            ...
            DoubleClick(event) { //触发双击事件
                ...
                if (intersects.length != 0) {
                    for (var item of intersects) {
                        if (item.object.name == '三亚' || item.object.name == '盐城' || item.object.name == '拉萨') { //找到第一个不等于空的模型 就是自定义最近的模型
                            ...
                            this.$confirm('是否切换场景?', '提示', {
                                ...
                                if (item.object.name == '三亚') {
                                    this.changeScene('sanya'); //改变页面场景
                                } else if (item.object.name == '盐城') {
                                    this.changeScene('yancheng'); //改变页面场景
                                } else if (item.object.name == '拉萨') {
                                    this.changeScene('lasa'); //改变页面场景
                                }
                            }).catch(() => {
                                this.action.paused = false; //开启旋转
                            });
            ...
            changeScene(type) {
                ...
                var textureLoader = new THREE.TextureLoader(); //创建纹理贴图		
                if (type == 'sanya') {//切换到三亚
                    img = textureLoader.load(require('../../public/img/sanya1.jpg'));
                    canvasText = this.getcanvers('盐城'); //生成一个canvers文字图案对象	
                    names = '盐城';
                } else if (type == 'yancheng') { //返回盐城
                    img = textureLoader.load(require('../../public/img/yancheng.jpg'));
                    canvasText = this.getcanvers('拉萨'); //生成一个canvers文字图案对象	
                    names = '拉萨';
                } else if (type == 'lasa') { //切换回拉萨
                    img = textureLoader.load(require('../../public/img/lasa.jpg'));
                    canvasText = this.getcanvers('三亚'); //生成一个canvers文字图案对象	
                    names = '三亚';
                }
                ...
                for (var item of this.scene.children[2].children) {
                    ...
                    } else if (item.name == '三亚' || item.name == '盐城' || item.name == '拉萨') {
                        var texture = new THREE.CanvasTexture(canvasText);
                        var materialText = new THREE.MeshPhongMaterial({
                            map: texture, // 设置纹理贴图
                            side: THREE.DoubleSide, //双面渲染
